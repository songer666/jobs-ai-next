...
const ResumeGeneratorLayoutContext = createContext<ResumeGeneratorLayoutContextType | null>(null);

export function useResumeGeneratorLayout() {
    const context = useContext(ResumeGeneratorLayoutContext);
    if (!context) {
        throw new Error('useResumeGeneratorLayout must be used within ResumeGeneratorLayout');
    }
    return context;
}
...
export default function ResumeGeneratorLayout({ children }: ResumeGeneratorLayoutProps) {
    ...
    return (
        <ResumeGeneratorLayoutContext.Provider value={{ 
            sidebarOpen, 
            toggleSidebar, 
            addNewResume, 
            newResumeId,
