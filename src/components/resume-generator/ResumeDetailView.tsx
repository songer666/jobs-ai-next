...
import { ArrowLeft, FileDown, Loader2, FileText, Calendar, RefreshCw, Settings, Trash2, AlertCircle } from 'lucide-react';
import { useRouter, useSearchParams } from 'next/navigation';
import { AlertDialog } from '@heroui/react';
import { toast } from 'sonner';
import { config, getApiUrl } from '@/lib/config';
import { getStatusLabel, type Resume, generateResume, DEFAULT_STYLE_PROMPT, useDeleteResume } from '@/api/resume-generator';
...
async function fetchResumeDetail(id: string): Promise<Resume> {
    const response = await fetch(getApiUrl(config.api.resumeGenerator.resumeDetail(id)), {
        credentials: 'include',
    });
    if (!response.ok) {
...
export function ResumeDetailView({ resumeId }: ResumeDetailViewProps) {
    ...
    // 自动生成逻辑：如果 URL 参数包含 autoGenerate=true，且简历内容为空，自动开始生成
    useEffect(() => {
        const autoGenerate = searchParams.get('autoGenerate');
        if (autoGenerate === 'true' && resume && !resume.content && !isRegenerating) {
            // 移除 URL 参数
            router.replace(`/dashboard/resume-generator/${resumeId}`);
            // 自动开始生成
            handleRegenerate();
        }
    }, [resume, searchParams, resumeId, router, isRegenerating]);
    ...
    const convertToWordCompatible = (html: string): string => {
        ...
        result = result.replace(/background:\s*linear-gradient\([^)]+\)/gi, 'background-color: #1e3a5f');
        result = result.replace(/background-image:\s*linear-gradient\([^)]+\)/gi, 'background-color: #1e3a5f');
        ...
        <div className={styles.container}>
            ...
            <div className={styles.card}>
                <div className="flex items-start justify-between mb-4">
                    ...
                    <span className={`${styles.statusBadge} ${getStatusClass(resume.status)}`}>
                            ...
                            <div className="flex items-start gap-4 pb-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-white/70 leading-relaxed">
                                        确定要永久删除 <span className="text-white font-medium">{resume?.name || '此简历'}</span> 吗？此操作无法撤销。
                                    </p>
                                </div>
                            </div>
