...
import { useDeleteResume, getStatusLabel, type Resume } from '@/api/resume-generator';
    ...
    if (!resumes || resumes.length === 0) {
        return (
            <div className={styles.emptyState}>
                ...
                <div className={styles.emptyDesc}>点击上方按钮开始创建你的第一份 AI 简历</div>
            </div>
        );
    }
            ...
            {resumes.map((resume) => (
                <div key={resume.id} className={styles.resumeCard}>
                    ...
                    <div className={styles.resumeCardMeta}>
                        {resume.stylePrompt && <span className="text-white/50 text-xs truncate max-w-[200px]">样式已设置</span>}
                    </div>
                    
                    <div className={styles.resumeCardActions}>
                        <button
                            type="button"
                            onClick={() => router.push(`/dashboard/resume-generator/${resume.id}`)}
