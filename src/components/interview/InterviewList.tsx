...
const styles = {
  ...
  usageWrapper:
    "flex items-center gap-2 px-4 py-2 bg-white/5 rounded-lg border border-white/10",
        ...
        <div className={styles.controls}>
          {usage && usage.limit > 0 && (
            <div className={styles.usageWrapper}>
              ...
              >
                <RefreshCw
                  className={`w-4 h-4 text-primary cursor-pointer ${isRefreshing ? "animate-spin" : ""}`}
                />
              </button>
              <Mic className="w-4 h-4 text-primary" />
              <span className={styles.usageText}>
                {locale === "zh-CN" ? "今日剩余" : "Remaining"}:{" "}
