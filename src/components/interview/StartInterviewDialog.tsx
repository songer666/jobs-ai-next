...
import { useCreateInterview, type InterviewLanguage, type AIModel } from '@/api/interview';
...
const styles = {
    ...
    iconWrapper: 'w-12 h-12 rounded-full bg-gradient-to-r from-[#fd409a] to-[#f5a867] flex items-center justify-center',
    ...
    startButton: 'px-6 py-2.5 gradient-btn text-white rounded-lg font-medium flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100',
};

export function StartInterviewDialog({ isOpen, onOpenChange }: StartInterviewDialogProps) {
    const t = useTranslations('interview');
    const { data: positions, isLoading: positionsLoading } = useGoalPositions(true);
    const createMutation = useCreateInterview();
    const [selectedJobId, setSelectedJobId] = useState<string>('');
    const [selectedLanguage, setSelectedLanguage] = useState<InterviewLanguage>('zh');
                ...
                <Modal.Container>
                    <Modal.Dialog className={styles.dialog}>
                        ...
                        <div className={styles.body}>
                            <div>
                                <label className={styles.label}>{t('startDialog.selectPosition')}</label>
                                <select
                                    value={selectedJobId}
                                ...
                                >
                                    ...
                                    {positions?.map((pos) => (
                                        <option key={pos.id} value={pos.id} className={styles.option}>
                            ...
                            <div>
                                <label className={styles.label}>{t('startDialog.selectLanguage')}</label>
                            ...
                            <div>
                                ...
                                >
                                    {modelOptions.map((model) => (
                                        <option key={model.value} value={model.value} className={styles.option}>
