...
export function InterviewSession({ interviewId }: InterviewSessionProps) {
  ...
  const {
    ...
    start: startTimer,
    stop: stopTimer,
  } = useInterviewTimer(interviewId, false);
      ...
      getChatMessages(interviewId)
        .then((savedMessages) => {
          ...
          if (interview.status !== "completed" && !exceeded) {
            startTimer();
            const aiCount = savedMessages.filter(
              (m) => m.role === "assistant",
            ).length;
            const userCount = savedMessages.filter(
              (m) => m.role === "user",
            ).length;
        ...
        <AlertDialog.Backdrop variant="blur">
          <AlertDialog.Container>
            <AlertDialog.Dialog className="sm:max-w-[420px] bg-[#1a1625] border border-white/10">
              ...
              <AlertDialog.Header>
                <AlertDialog.Icon
                  status={aiMessageCount < 10 ? "warning" : "accent"}
                />
                <AlertDialog.Heading>
              ...
              <AlertDialog.Body>
                {aiMessageCount < 10 ? (
                  <p className="text-yellow-400">
                    {t("session.endConfirm.warningMessage", {
