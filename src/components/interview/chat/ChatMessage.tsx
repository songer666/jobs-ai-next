...
const styles = {
    ...
    aiMessage: 'bg-gradient-to-r from-[#fd409a]/20 to-[#f5a867]/20 border border-white/10 text-white',
    ...
    playButton: 'p-1 rounded-full hover:bg-white/10 transition-colors cursor-pointer',
    playButtonActive: 'p-1 rounded-full bg-white/20 transition-colors cursor-pointer',
    playIcon: 'w-4 h-4',
    messageContent: 'prose prose-invert prose-sm max-w-none',
};

export function ChatMessage({ role, content, isStreaming, language = 'zh-CN' }: ChatMessageProps) {
    ...
    return (
        <div className={`${styles.messageWrapper} ${isUser ? styles.messageWrapperUser : styles.messageWrapperAi}`}>
                    ...
                    {isUser ? (
                        <>
                            {isSupported && content && (
                                <button
                                    onClick={handleSpeak}
                                    className={isSpeaking ? styles.playButtonActive : styles.playButton}
