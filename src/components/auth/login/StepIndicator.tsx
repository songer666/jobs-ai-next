...
export default function StepIndicator({ currentStep, onStepClick, canGoBack = false }: StepIndicatorProps) {
  ...
  return (
    <div className={authStyles.steps.wrapper}>
      ...
      <div className={currentStep !== 'credentials' ? authStyles.steps.lineActive : authStyles.steps.line} />
      <div className={`${authStyles.steps.step} ${currentStep === 'otp' ? authStyles.steps.active : authStyles.steps.inactive}`}>
        2
      </div>
    </div>
  );
