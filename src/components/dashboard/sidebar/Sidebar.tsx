...
const styles = {
  // 桌面端侧边栏
  sidebar:
    "bg-[#0d0a1f] border-r border-white/10 flex-col transition-all duration-300",
  sidebarExpanded: "w-64",
  sidebarCollapsed: "w-16",
  // 移动端覆盖层
  overlay: "fixed inset-0 bg-black/50 z-40 md:hidden",
  mobileSidebar:
    "fixed left-0 top-0 h-full w-64 bg-[#0d0a1f] border-r border-white/10 flex flex-col z-50 md:hidden",
  // Logo
  logo: {
  ...
  closeBtn:
    "w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-white/60 hover:bg-white/10 hover:text-white transition-colors md:hidden",
};
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* 桌面端侧边栏 */}
      <aside
        className={`${styles.sidebar} ${isCollapsed ? styles.sidebarCollapsed : styles.sidebarExpanded} hidden md:flex`}
      >
        <div
          className={
            isCollapsed ? styles.logo.wrapperCollapsed : styles.logo.wrapper
          }
        >
          <Link
            href="/"
            className={
              isCollapsed ? styles.logo.wrapperCollapsed : styles.logo.container
            }
          >
            {isCollapsed ? (
              <Logo size={40} />
