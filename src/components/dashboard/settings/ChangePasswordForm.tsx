...
export function ChangePasswordForm() {
  ...
  const renderStepIndicator = () => (
    <div className={styles.stepIndicator}>
      <div
        className={`${styles.stepDot} ${step === "email" || step === "otp" || step === "reset" || step === "success" ? styles.stepDotActive : styles.stepDotInactive}`}
      />
      <div className={styles.stepLine} />
      <div
        className={`${styles.stepDot} ${step === "otp" || step === "reset" || step === "success" ? styles.stepDotActive : styles.stepDotInactive}`}
      />
      <div className={styles.stepLine} />
      <div
        className={`${styles.stepDot} ${step === "reset" || step === "success" ? styles.stepDotActive : styles.stepDotInactive}`}
      />
    </div>
  );
    ...
    <div className={styles.card}>
      ...
      {step === "email" && (
        <div>
          ...
          <p className={styles.description}>{t("steps.emailDesc")}</p>
      ...
      {step === "otp" && (
        <div>
          ...
          <p className={styles.description}>{t("steps.otpDesc", { email })}</p>
          <div className="space-y-4">
            <OTPInput
              value={otp}
              onChange={setOtp}
            ...
            <div className="flex justify-between items-center">
              ...
              >
                {countdown > 0
                  ? t("actions.resendCountdown", { countdown })
                  : t("actions.resend")}
              </Button>
            </div>
            <Button
      ...
      {step === "reset" && (
        <div>
          ...
          <p className={styles.description}>{t("steps.resetDesc")}</p>
          <Form
            className="space-y-4"
            onSubmit={(e) => {
              e.preventDefault();
