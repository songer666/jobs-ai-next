...
import { Target, FileText, BarChart3, HelpCircle, Calendar, ChevronRight, Loader2, Sparkles, Clock } from 'lucide-react';
...
const styles = {
    ...
    welcome: {
        ...
        gradient: 'absolute inset-0 bg-gradient-to-r from-[#fd409a]/20 to-[#f5a867]/20',
    ...
    section: {
        wrapper: 'bg-[#1a1528] border border-white/10 rounded-xl sm:rounded-2xl p-4 sm:p-6',
        ...
        viewAll: 'text-[#fd409a] text-xs sm:text-sm hover:text-[#f5a867] flex items-center gap-1',
    },
    actions: {
        grid: 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4',
    ...
    list: {
        ...
        title: 'text-sm sm:text-base text-white font-medium truncate max-w-[105px] sm:max-w-[250px] md:max-w-[300px] lg:max-w-[400px] xl:max-w-5xl',
...
export default function DashboardOverview() {
    ...
    const stats = data ? [
        { icon: Target, value: data.stats.interviewCount, label: t('stats.interview'), color: 'bg-blue-500/20 text-blue-400' },
        { icon: FileText, value: data.stats.resumeCount, label: t('stats.resume'), color: 'bg-purple-500/20 text-purple-400' },
        { icon: BarChart3, value: data.stats.analysisCount, label: t('stats.analysis'), color: 'bg-pink-500/20 text-pink-400' },
        { icon: HelpCircle, value: data.stats.questionCount, label: t('stats.questions'), color: 'bg-green-500/20 text-green-400' },
    ] : [];

    const actions = [
        { icon: Target, title: t('actions.interview.title'), desc: t('actions.interview.desc'), href: '/dashboard/interview', color: 'bg-blue-500/20 text-blue-400' },
        { icon: FileText, title: t('actions.resume.title'), desc: t('actions.resume.desc'), href: '/dashboard/resume-generator', color: 'bg-purple-500/20 text-purple-400' },
        { icon: BarChart3, title: t('actions.analysis.title'), desc: t('actions.analysis.desc'), href: '/dashboard/resume-analyzer', color: 'bg-pink-500/20 text-pink-400' },
        { icon: HelpCircle, title: t('actions.questions.title'), desc: t('actions.questions.desc'), href: '/dashboard/questions', color: 'bg-green-500/20 text-green-400' },
    ];

    // 转换最近活动数据
    const getActivityConfig = (type: 'interview' | 'resume' | 'analysis' | 'question') => {
        const configs = {
            interview: { typeLabel: t('activity.types.interview'), icon: Target, color: 'bg-blue-500/20 text-blue-400', basePath: '/dashboard/interview' },
            resume: { typeLabel: t('activity.types.resume'), icon: FileText, color: 'bg-[#fd409a]/20 text-[#fd409a]', basePath: '/dashboard/resume-generator' },
            analysis: { typeLabel: t('activity.types.analysis'), icon: BarChart3, color: 'bg-pink-500/20 text-pink-400', basePath: '/dashboard/resume-analyzer' },
            question: { typeLabel: t('activity.types.question'), icon: HelpCircle, color: 'bg-green-500/20 text-green-400', basePath: '/dashboard/questions' },
        };
        return configs[type];
    };
                    ...
                    <div className={styles.section.wrapper}>
                        ...
                        <div className={styles.actions.grid}>
                            {actions.map((action, index) => (
                                <Link key={index} href={action.href} className={styles.actions.card}>
                            ...
                            <>
                                <div className={styles.usage.item}>
                                    <div className={styles.usage.label}>
                                        <span className={styles.usage.labelText}>{t('usage.interview')}</span>
                                    ...
                                    <div className={styles.usage.bar}>
                                        <div
                                            ...
                                            style={{ width: `${Math.min((data.usage.interview.used / data.usage.interview.limit) * 100, 100)}%` }}
                                        />
                                    </div>
                                </div>
                                <div className={styles.usage.item}>
                                    <div className={styles.usage.label}>
                                        <span className={styles.usage.labelText}>{t('usage.resume')}</span>
                                        <span className={styles.usage.labelValue}>
                                    ...
                                    <div className={styles.usage.bar}>
                                        <div
                                            ...
                                            style={{ width: `${Math.min((data.usage.generate.used / data.usage.generate.limit) * 100, 100)}%` }}
                                        />
                                    </div>
                                </div>
                                <div className={styles.usage.item}>
                                    <div className={styles.usage.label}>
                                        <span className={styles.usage.labelText}>{t('usage.analysis')}</span>
                                        <span className={styles.usage.labelValue}>
                                            {data.usage.analyze.used} / {data.usage.analyze.limit}
                                        </span>
                                    </div>
                                    <div className={styles.usage.bar}>
                                        <div
                                            className={`${styles.usage.fill} ${getUsageColor((data.usage.analyze.used / data.usage.analyze.limit) * 100)}`}
                                            style={{ width: `${Math.min((data.usage.analyze.used / data.usage.analyze.limit) * 100, 100)}%` }}
                                        />
                                    </div>
                                </div>
                                <div className={styles.usage.item}>
                                    <div className={styles.usage.label}>
                                        <span className={styles.usage.labelText}>{t('usage.questions')}</span>
                                        <span className={styles.usage.labelValue}>
                                    ...
                                    <div className={styles.usage.bar}>
                                        <div
                                            ...
                                            style={{ width: `${Math.min((data.usage.question.used / data.usage.question.limit) * 100, 100)}%` }}
                                        />
                                    </div>
                                </div>
                            </>
                        )}
                        <Link href="/dashboard/billing" className="block mt-4 text-center text-transparent bg-clip-text bg-gradient-to-r from-[#fd409a] to-[#f5a867] text-sm hover:opacity-80">
                            {t('usage.upgrade')}
                        </Link>
                    </div>
                </div>
                    ...
                    <div className={styles.list.wrapper}>
                        {recentActivities.map((item, index) => (
                            <Link key={index} href={item.href} className={styles.list.item}>
                                ...
                                {item.score !== null && (
                                    <div className={`${styles.list.score} ${getScoreColor(item.score)}`}>
                                        {item.score}{t('activity.score')}
                                    </div>
                                )}
