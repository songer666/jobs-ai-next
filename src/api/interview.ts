...
export function useInterviews() {
  ...
  const query = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const hasEvaluating = interviews.some(
        (i: Interview) => i.status === "evaluating",
      );
...
export async function generateQuestion(
  ...
  const response = await fetch(
    getApiUrl(config.api.interview.ai.generateQuestion),
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function submitAnswer(
  ...
  const response = await fetch(
    getApiUrl(config.api.interview.ai.submitAnswer),
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export function useCompleteInterview() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await fetch(
        getApiUrl(`${config.api.interview.base}/complete`),
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
export async function getChatMessages(
  ...
  const response = await fetch(
    getApiUrl(`${config.api.interview.detail(interviewId)}/messages`),
    {
      credentials: "include",
    },
  );
...
export async function getInterviewTimer(
  interviewId: string,
): Promise<TimerResponse> {
  const response = await fetch(
    getApiUrl(`${config.api.interview.detail(interviewId)}/timer`),
    {
      credentials: "include",
    },
  );
  ...
  if (!response.ok) {
    return {
      success: false,
      elapsedTime: 0,
      maxDuration: MAX_INTERVIEW_DURATION,
    };
  }
...
export async function startInterviewTimer(
  interviewId: string,
): Promise<TimerResponse> {
  const response = await fetch(
    getApiUrl(config.api.interview.timer.start(interviewId)),
    {
      method: "POST",
      credentials: "include",
    },
